...
// Colors are interchangeable, so we can assume we will use the colors 0..nrOfColors-1. 
// Number of colors used is then equal to the maximal color used. 
//dexpr int nrOfColors = max(c in Countries) color[c];
